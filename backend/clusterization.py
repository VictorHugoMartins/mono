...
def add_labels_to_table_view(table_view, labels):
	table_view["columns"].append({'value': 'cluster', 'label': "Cluster", 'type': 'number'})
...
# hierarquico
def run_birch(data, table_view, n_clusters=3):
	df_scaled = prepare_mixed_data(data)
...
def cluster_data(clusterization_method='none', data=None, table_view=None, parameters=None):
	if (clusterization_method == 'kmodes'):
		...
		return run_dbscan(data, table_view, parameters["eps"], parameters["min_samples"])
	elif (clusterization_method == 'birch'):
		return run_birch(data, table_view, parameters["n_clusters"])
	return table_view