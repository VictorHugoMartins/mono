...
entorno = ['Bairro Da Lagoa', 'Bauxita', 'Morro Do Cruzeiro',
           'Saramenha De Cima', 'Vila Aparecida', 'Vila Itacolomy']
...
def create_columns_with_bathroom_propriety(site, da):
    ...
    da['republica'] = [1 if 'república' in str(
        x).lower() else 0 for x in da['name']]

    if site == 'Airbnb':
        da['private_bathroom'] = [1 if 'privado' in str(
            x).lower() else 0 for x in da['bathroom']]
        da['shared_bathroom'] = [1 if 'compartilhado' in str(
            x).lower() else 0 for x in da['bathroom']]
    elif site == 'Booking':
        da['private_bathroom'] = [1 if 'banheiro privativo' in str(
            x).lower() else 0 for x in da['name']]
        da['shared_bathroom'] = [1 if 'banheiro compartilhado' in str(
            x).lower() else 0 for x in da['name']]
...
def create_columns_with_comodities(da, lista):
    for cname in lista:
        traducao = cname[0]
        original = cname[1]
        da[original] = [1 if (original in x or traducao in x)
                        else 0 for x in da['comodities']]
    return da
...
def prepare_airbnb(da, table='Airbnb'):
    ...
    da['price'] = [x * 5.05 if x != '-1' and y ==
                   'USD' else x for x, y in zip(da['price'], da['currency'])]
    da['currency'] = ['BRL' if x != '.' else x for x in da['currency']]

    # calculate the price per capita

    da['price_pc'] = [x / y if x != '-1' else x for x,
                      y in zip(da['price'], da['accommodates'])]
    da['comodities'] = [prepare_text(x) for x in da['comodities']]

    da['qtd_comodities'] = [len(x.split(',')) for x in da['comodities']]

    da['count_host_id'] = [
        da.query('host_id==' + str(x))['Unnamed: 0'].count() for x in da['host_id']]
    da['hotel_id'] = [-1 for x in da['Unnamed: 0']]
    da['count_hotel_id'] = [-1 for x in da['Unnamed: 0']]

    da['region'] = [define_region(sub, lat, lng) for sub, lat, lng in zip(
        da['sublocality'], da['latitude'], da['longitude'])]
    ...
    da = da.apply(lambda x: x.fillna(x.mean())
...
def prepare_booking(db, table='Booking'):
    ...
    db['price'] = [get_price(x, y)
                   for x, y in zip(db['price'], db['qtd_rooms'])]
    ...
    db['price'] = [float(str(x).split('\n')[0])
                   if x is not None else 0 for x in db['price']]

    db['price_pc'] = [x / y if x != '-1' else x for x,
                      y in zip(db['price'], db['accommodates'])]
    # if x != '-1' else float(x)
    db['os'] = [float(x / 2.0) for x in db['overall_satisfaction']]
    ...
    db['region'] = [define_region(sub, lat, lng) for sub, lat, lng in zip(
        db['sublocality'], db['latitude'], db['longitude'])]
    ...
    db['name'] = ['Quarto não identificado' if str(
        x) == 'nan' else x for x in db['room_name']]
    db['name'] = ['{}, {}'.format(room, hotel)
                  for room, hotel in zip(db['room_name'], db['name'])]

    db = db.drop(columns=['reviews', 'currency', 'images', 'state', 'room_name', 'address',
                          'city', 'state', 'country', 'popular_facilidades'])

    db = db.apply(lambda x: x.fillna(x.mean())
                  if x.dtype.kind in 'biufc' else x.fillna('.'))
    return db
...
def get_clustered_data(writer, table, city, total_clusters, original_data, data, rd):
    ...
    clustered_data['total_clusters'] = [
        total_clusters for x in clustered_data['Unnamed: 0']]
    ...
    if total_clusters == 1:  # just export the data
        ...
        if 'cluster' in clustered_data.columns:
            clustered_data = clustered_data.drop(
...
def prepare_data(table, dairbnb=None, dbooking=None):
    if table == 'Airbnb':
        ...
        comodities_airbnb = get_individual_comodities(
            data_airbnb['comodities'])
        data_airbnb = create_columns_with_bathroom_propriety(
            'Airbnb', data_airbnb)

        data_booking = use_distinct_rooms(data_booking)

        data_booking = prepare_booking(data_booking, table=table)
        comodities_booking = get_individual_comodities(
            data_booking['comodities'])
        data_booking = create_columns_with_bathroom_propriety(
...
def compare_sites(writer, table, area, data, rd=1000):
    ...
    for total_clusters in range(1, 4):
        original_data = data
        get_clustered_data(writer, table, area, total_clusters,
                           filled_data, filled_data, rd)
